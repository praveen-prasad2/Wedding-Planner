...
export async function GET() {
  ...
  const categories = await Category.find({ userId: session.user.id }).sort({ createdAt: 1 }).lean();
...
export async function POST(request: Request) {
  ...
  await connectToDatabase();
  const category = await Category.create({ ...parsed.data, userId: session.user.id });
